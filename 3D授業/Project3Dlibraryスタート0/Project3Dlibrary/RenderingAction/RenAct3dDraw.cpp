...
void RenderAction_3D_Cube::Action()
{
	...
	//描画内容
	{
		...
		// ビュー・プロジェクション・ワールド行列
		m_r += 0.1f;
		XMMATRIX view = XMMatrixLookAtLH(XMVECTOR{ 0.0f, 2.0f, -10.5f }, XMVECTOR{ 0.0,0.0f,0.0f }, XMVECTOR{ 0.0f, 1.0f, 0.0f });
		...
		//デスプリタテーブルに設定
		Com->GetList()->SetGraphicsRootDescriptorTable(1, Cuble3D->m_tex->GetDescriptorHeap()->GetGPUDescriptorHandleForHeapStart());
