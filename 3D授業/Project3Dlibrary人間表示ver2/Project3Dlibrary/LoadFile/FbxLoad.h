...
namespace FBX_FILE
{
	...
	{
	public:
		int32_t mat_id[4];//影響を受ける行列ID
		float weight[4];	  //影響を受けるボーン行列の影響度（重み）
	};
	...
	{
	public:
		int32_t	mIndex;			//ボーンID
		float	mInvBindMat[16];//ボーンのインバースバインド行列（原点に戻す行列）
	};
	...
	public:
		...
		void ParseMesh(const aiMesh* pSrcMesh, Mesh& mesh)
		{
			...
			//骨情報がある場合
			if (pSrcMesh->mNumBones != 0)
			{
				int num_bones = pSrcMesh->mNumBones;

				for (size_t i = 0; i < num_bones; i++)
				{
					//データを追加
					mesh.bone.insert(make_pair(pSrcMesh->mBones[i]->mName.C_Str(), Bone()));
					//ボーン番号
					mesh.bone[pSrcMesh->mBones[i]->mName.C_Str()].mIndex = mesh.bone.size() - 1;
					//ボーンのインバースバインドポーズ行列を取得
					auto m = &pSrcMesh->mBones[i]->mOffsetMatrix;
					float* mat = mesh.bone[pSrcMesh->mBones[i]->mName.C_Str()].mInvBindMat;
					mat[ 0] = m->a1; mat[ 1] = m->b1; mat[ 2] = m->c1; mat[ 3] = m->d1;
					mat[ 4] = m->a2; mat[ 5] = m->b2; mat[ 6] = m->c2; mat[ 7] = m->d2;
					mat[ 8] = m->a3; mat[ 9] = m->b3; mat[10] = m->c3; mat[11] = m->d3;
					mat[12] = m->a4; mat[13] = m->b4; mat[14] = m->c4; mat[15] = m->d4;
				...
				//スキン頂点データのメモリ確保
				mesh.skin.resize(pSrcMesh->mNumVertices);

				//ボーンの数
				for (size_t i = 0; i < num_bones; i++)
				{
					//ボーンが所有するweightの影響とその対象頂点インデックス
					for (size_t j = 0; j < pSrcMesh->mBones[i]->mNumWeights; j++)
					{
						int32_t		id = mesh.bone[pSrcMesh->mBones[i]->mName.C_Str()].mIndex;//ボーンID
						int32_t	   vid = pSrcMesh->mBones[i]->mWeights[j].mVertexId;		  //頂点番号（ボーンIDを入れる頂点の番号）
						float	     w = pSrcMesh->mBones[i]->mWeights[j].mWeight;			  //重み

						//頂点番号の場所にボーンの番号とweight値を入れる
						for (size_t k = 0; k < 4; k++)
						{
							if (mesh.skin[vid].weight[k] == 0.0f)
							{
								mesh.skin[vid].mat_id[k] = id;
								mesh.skin[vid].weight[k] = w;
								break;
							}
